...
1. **Stack & Framework (Astro v7 + PHP 8):**
   * El proyecto utiliza **Astro v7** (`"astro": "^7.1.1"`) para el frontend estático e internacionalización.
   * El backend API utiliza un Front Controller MVC en PHP 8 (`public/index.php` → `src/Controllers/`) optimizado para despliegue nativo en Hostinger.
...
3. **Seguridad Absoluta y Cero Secretos:**
...
4. **Abstracción Justa y Equilibrada (Anti-Falta de Abstracción):**
...
5. **Simplicidad Resiliente (Anti-Sobreingeniería):**
   ...
   * A pesar de la simplicidad, incluye un manejo de errores robusto para fallos clásicos (red, nulos, BD) y centraliza las excepciones mediante `HttpException` que no filtre datos sensibles en producción.

6. **Estrategia Híbrida de Auditoría y Testing Multiplataforma:**
   * **Análisis Estático:** `astro check` para tipos TypeScript y `PHPStan` para análisis de tipos en PHP.
   * **Pruebas de Backend:** Pruebas unitarias y de integración HTTP compatibles con Windows, Linux y macOS usando `proc_open` con descriptores nulos cruzados (`NUL` / `/dev/null`).
   * **Pruebas E2E & Calidad:** Playwright para flujos de reserva en navegador real y auditorías de rendimiento/SEO.

7. **Ejecución Inteligente y Conexión MCP (La más importante):**
   * **SIEMPRE USA CONTEXT7, SEQUENTIAL THINKING** y demás conexiones MCP (Graphify) para estructurar el razonamiento por pasos antes de dar resultados globales.
